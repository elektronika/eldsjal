...
<?php
//if session("userType") < 5 then response.Redirect("main.php?message=Funktionen under ombyggnad, f&ouml;rs&ouml;k igen senare!")
...
<div class="regWindow1"> 
  ...
  $locationss = $locations;
	...
	$sql = "insert into history (action, userid, nick, message, [date], security) values ('eventadd',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." h&aring;ller p&aring; och skapa ny aktivitet', getdate(), 0)";
	$conn->execute( $sql );
	?>
	</select>L&Auml;N	
	<div class="boxtext2">
...
elseif( isset( $_GET['mode'] ) && $_GET['mode'] == "getImageName" ) {
  ...
  $sql = "select title from calendarevents where eventid = ".intval( $_GET['eventid'] );
	...
	$sql = "insert into history (action, userid, nick, message, [date], security) values ('eventimage',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." v&auml;ljer nu passande aktivitetsbild till \"".$result['title']."\"', getdate(), 0)";
	$conn->execute( $sql );
	$result = null;
}
elseif( isset( $_GET['mode'] ) && $_GET['mode'] == "upcoming" ) {
	...
	$sql = "insert into history (action, userid, nick, message, [date], security) values ('eventoverview',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." kollar vilka aktiviteter som &auml;r kommande', getdate(), 0)";
	$conn->execute( $sql );
...
elseif( isset( $_GET['mode'] ) && $_GET['mode'] == "userList" ) {
	...
	if( !is_array( current( $eventList ) ) ) 
		...
		foreach( $eventLists as $eventList ) {
			...
			print "<tr><td valign=\"top\" width=\"70\">".$activityDate."</td><td><a href=\"calendarView.php?eventid=".$eventList['eventid']."&yy=".$eventList['yyyy']."&mm=".$eventList['mm']."&dd=".$eventList['dd']."\" class=\"a2\">".$eventList['title'].", ".$eventList['locationname']."</a></td></tr>";
...
elseif( isset( $_GET['mode'] ) && $_GET['mode'] == "update" ) {
	...
	<?php
		...
		$sql = "insert into history (action, userid, nick, message, [date], security) values ('eventedit',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." &auml;ndrar i aktiviteten \"".$cevent['title']."\"', getdate(), 0)";
		$conn->execute( $sql );
		$locations = null;
	}
	else {
	...
	if( $_SESSION['userid'] != "" ) {
		...
		if( isset( $_GET['eventid'] ) && $_GET['eventid'] != "" ) {
			...
			if( $validation ) {
				...
				<td valign="top" class="calendarViewTopRight">
				<?php 
          print $datum."<br>";
        ...
        <tr> 
          <td colSpan="2" valign="top" align="left" class="calendarViewContentLeft">
