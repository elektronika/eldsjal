...
if( !in_array( $_FILES['file1']['type'], array( 'image/gif', 'image/jpg', 'image/jpeg', 'image/png', 'image/bmp' ) ) ) {
	header( "Location: "."imageUpload.php?message=Du f&aring;r bara ladda upp bilder av typerna: BMP, JPG, PNG, GIF, PNG och TIFF" );
	exit( );
}
...
if( move_uploaded_file( $_FILES['file1']['tmp_name'], $strUploadPath.'/'.$_FILES['file1']['name'] ) ) {
	...
	header("Location: "."imageUpload.php?message=Ingen bild blev uppladdad, f&ouml;rs&ouml;k igen");
	exit();
}
// pint_r( $file );
// exit( );
...
$originalImagePath = $application['originalimagepath']."/".$imageName['imageid'].'.'.$file['extension'];
...
// "Stor" version
$imageTransform->targetFile = '../uploads/galleryImages/'.$imageName['imageid'].'.'.$file['extension'];
...
// Thumbnail
$imageTransform->targetFile = '../uploads/galleryImages/tn_'.$imageName['imageid'].'.'.$file['extension'];
...
<tr><td>
<img border=1 src="images/galleryImages/tn_<?php echo $imageName['imageid'];?>.jpg"> 
</td>
<td>
