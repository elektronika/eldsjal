...
if( isset( $_GET['mode'] ) && $_GET['mode'] == "readTopic" ) {
	...
	if(( intval( $_GET['threadid'] == 122 ) ) ) {
		...
		$sql = "select forumcategory.forumsecuritylevel, forumtopics.locked, forummessages.messageid , forummessages.posterid, forumtopics.topicname, forumtopics.topicid, users.username, users.hasimage, users.userid, forummessages.messagedate, forummessages.message from forummessages inner join users on users.userid = forummessages.posterid inner join forumtopics on forummessages.topicid = forumtopics.topicid inner join forumcategory on forumcategory.forumcategoryid = forumtopics.forumcategoryid where forummessages.topicid = ".intval( $_GET['threadid'] )." order by forummessages.messagedate asc";
	}
	...
	if( $_SESSION['userid'] != "" ) {
		...
		if( $threads[0]['forumsecuritylevel'] > 1 ) 
			$seclevel = 5;
		$sql = "replace into history (action, userid, nick, message, [date], security) values ('forumpostview',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." studerar tr&aring;den \"".$threads[0]['topicname']."\" i forumet', getdate(), ".$seclevel.")";
		$conn->execute( $sql );
	}
	...
	foreach( $threads as $thread ) {
		...
		print "<tr class=".$format."><td rowspan=2><a class=\"a2\" href=\"userPresentation.php?userid=".$thread['posterid']."\">";
		...
		if( intval( $_SESSION['usertype'] ) >= intval( $application['forumadmin'] ) || $_SESSION['userid'] == intval( $thread['posterid'] ) ) {
			print "<td><a href=\"forum.php?mode=updatePost&threadid=".$_GET['threadid']."&category=".$_GET['category']."&messageid=".$thread['messageid']."\"><img src=images/icons/edit.gif border=0></a>&nbsp;<a href=forumAct.php?mode=deletePost&messageid=".$thread['messageid']."&category=".$_GET['category']."&threadid=".$_GET['threadid']." onClick=\"return confirmSubmit('Vill du verkligen ta bort denna post?')\"><img src=images/icons/trashcan.gif border=0></a> ".timeSince(strtotime( $thread['messagedate'] ) )."<hr></td>";
		}
		else {
			print "<td> ".timeSince(strtotime( $thread['messagedate'] ) )."<hr></td>";
		}
		print "</tr><tr class=".$format."><td>".$message."<br/></td></tr>";
...
elseif( isset( $_GET['mode'] ) && $_GET['mode'] == "updatePost" ) {
	if( $_SESSION['usertype'] >= 1 ) {
		...
		$sql = "replace into history (action, userid, nick, message, [date], security) values ('forumpostupdate',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." &auml;ndrar sitt inl&auml;gg i \"".$resultss['topicname']."\" i forumet', getdate(), 0)";
		$conn->execute( $sql );
		if( !$resultss ) {
...
elseif( isset( $_GET['mode'] ) && $_GET['mode'] == "newTopic" ) {
	...
	-->
			...
			<form name="newTopic" method="post" action="forumAct.php?mode=addEntry" onSubmit="return CheckForumForm();">
			Rubrik<br/>
...
<?php
	...
	$sql = "replace into history (action, userid, nick, message, [date], security) values ('forumpostnewthread',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." h&aring;ller p&aring; och skriva en ny forumtr&aring;d i forumet', getdate(), 0)";
	$conn->execute( $sql );
}
...
elseif( isset( $_GET['mode'] ) && $_GET['mode'] == "lockTopic" ) {
	...
	if( intval( $locked['locked'] ) == 1 || $locked['locked'] == true ) {
		...
		$sql = "replace into history (action, userid, nick, message, [date], security) values ('forumthreadunlock',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." l&aring;ste just upp tr&aring;den \"".$locked['topicname']."\"', getdate(), 0)";
		$conn->execute( $sql );
		...
		$sql = "replace into history (action, userid, nick, message, [date], security) values ('forumthreadlock',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." l&aring;ste just tr&aring;den \"".$locked['topicname']."\"', getdate(), 0)";
		$conn->execute( $sql );
...
elseif( isset( $_GET['mode'] ) && $_GET['mode'] == "editTopic" ) {
	...
	$sql = "replace into history (action, userid, nick, message, [date], security) values ('forumthreadedit',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." h&aring;ller p&aring; och &auml;ndrar tr&aring;den \"".$topic['topicname']."\"', getdate(), 0)";
	$conn->execute( $sql );
	?>

			<form name="Topic" method="post" action="forumAct.php?mode=updateTopic&category=<?php   echo $_GET['category'];?>" onSubmit="return CheckForumForm();" id="Form3">
			Topic<br/>
...
elseif( isset( $_GET['category'] ) ) {
	...
	if( $categoryName ) {
		if( $_SESSION['userid'] != "" ) {
			//LOGGER
			$sql = "replace into history (action, userid, nick, message, [date], security) values ('forumcategoryview',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." kikar i forumkategorin \"".$categoryName['forumcategoryname']."\"', getdate(), ".$categoryName['forumsecuritylevel'].")";
			$conn->execute( $sql );
		}
		...
		if( $_SESSION['userid'] != "" ) {
			...
			if(( $topicLists == 0 ) && $sticky ) {
				...
				print "<tr><td>&nbsp;</td><td width=250>&Auml;mne</td><td>&nbsp;</td><td>senast svar</td><td width=30>svar</td><td>skapad av</td><td width=75>skapad den</td></tr>";
				$iCount = 1;
				...
				if( !empty( $stickys ) ) 
					...
					if( $iCount == 1 ) {
						...
						print "</td><td>".timeSince(strtotime($sticky['latestentry']))."</td><td><b>".$sticky['totalentrys']."</b></td><td><a class=a2 href=userPresentation.php?userid=".$sticky['userid'].">".$sticky['username']."</a></td><td>".date( 'Y-m-d', strtotime( $sticky['topicdate'] ) )."</td></span></tr>";
						$iCount = 0;
					}
					else {
						...
						print "</td><td>".timeSince(strtotime($sticky['latestentry']))."</td><td><b>".$sticky['totalentrys']."</b></td><td><a class=a2 href=userPresentation.php?userid=".$sticky['userid'].">".$sticky['username']."</a></td><td>".date( 'Y-m-d', strtotime( $sticky['topicdate'] ) )."</td></tr>";
						$iCount = $iCount + 1;
					}
				}
				$counter = 1;
				...
				foreach( $topicLists as $topicList ) {
					...
					if( $iCount == 1 ) {
						...
						print "</td><td>".timeSince(strtotime($topicList['latestentry']))."</td><td><b>".$topicList['totalentrys']."</b></td><td><a class=a2 href=userPresentation.php?userid=".$topicList['userid'].">".$topicList['username']."</a></td><td>".date( 'Y-m-d', strtotime( $topicList['topicdate'] ) )."</td></span></tr>";
						$iCount = 0;
					}
					else {
						...
						print "</td><td>".timeSince(strtotime($topicList['latestentry']))."</td><td><b>".$topicList['totalentrys']."</b></td><td><a class=a2 href=userPresentation.php?userid=".$topicList['userid'].">".$topicList['username']."</a></td><td>".date( 'Y-m-d', strtotime( $topicList['topicdate'] ) )."</td></tr>";
						$iCount = $iCount + 1;
					}
...
<?php
	...
	//LOGGER
	$sql = "replace into history (action, userid, nick, message, date, security) values ('forumsearch',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." s&ouml;ker i forumet', getdate(), 0)";
...
//}
	...
	if( $_SESSION['userid'] != "" ) {
		//LOGGER
		$sql = "replace into history (action, userid, nick, message, [date], security) values ('forumcategories',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." bligar &ouml;ver forumkategorierna', getdate(), 0)";
		$conn->execute( $sql );
	}
	print '<h2>Forum</h2>';
	print "<table border=0 width=600 cellpadding=3 cellspacing=3>";
	print "<tr><td>&nbsp;</td><td width=50>Kategori</td><td width=300>Beskrivning</td><td width=30>Tr&aring;dar</td><td>Senaste inl&auml;gg</td></tr>";
	...
	foreach( $categorys as $category ) {
		...
		print "<tr class=ForumTopic1 onmouseover=\"this.style.background='#cccc99';\" onmouseout=\"this.style.background='#e0d1bc';\"><span style:background-color = \"#e0d1bc\"><td class=ForumTopic1>".$unReadFlag."</td><td class=ForumTopic1><a class=a2 href=forum.php?category=".$category['forumcategoryid'].">".$category['forumcategoryname']."</a></td><td><a class=a3 href=forum.php?category=".$category['forumcategoryid'].">".$category['forumcategorydesc']."</a></td><td><b>".$category['forumcategorythreads']."</b></td><td>".timeSince(strtotime( $category['forumcategorylatestpost'] ) )."</td></span></tr>";
