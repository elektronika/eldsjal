...
$rs = $this->db->query( "SELECT DISTINCT(dd) FROM calendarevents WHERE mm = ".$mm." AND yyyy = ".$yyyy." AND private = 0 ORDER BY dd" )->result_array();
	...
	for( $y = 1; $y <= 7; $y = $y + 1 ) {
		...
		if( !( $rs == 0 ) ) {
			if( isset( $rs[$dag] ) ) {
				...
				$hovers = $this->db->query( $sql )->result_array();
...
// print "<a href=calendarView.php><img src='left.gif' width='15' height='10' border='0'></a></td>";
?>
          ...
          <td class="calendarIncreaseMonth"> 
           <?php 
// print "<a href=calendarView.php><img src='right.gif' width='15' height='10' border='0'></a>";
?>
          </td>
        </tr>
      </table>