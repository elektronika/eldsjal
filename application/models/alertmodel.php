<?php
class AlertModel extends AutoModel {
  	protected $alerts;
	...
	public function count($type = null) {
	    if( ! is_array($this->alerts) && $this->session->isLoggedIn()) {
			self::$alerts = Array();
			$alerts = $this->db->query("SELECT type,count(type) AS count FROM alert WHERE uid='{$this->session->userId()}' GROUP BY type")->result_array();
	...
	*/
	...
	protected function get_gb_alerts($user_id) {
		...
		// Borde inte det vara $username för $user_id?
		$userslug = $this->util->slugify($this->session->username());
	...
	protected function get_event_alerts($user_id) {
		$alerts = $this->db->query("SELECT title, fulldate AS date, u.userid, u.username, c.eventid AS id FROM calendarevents AS c JOIN users AS u ON u.userid = c.userid JOIN calendarnotify AS cn ON c.eventid = cn.eventid WHERE cn.userid = {$user_id}")->result();
		foreach($alerts as $alert) {
			$alert->href = '/calendar/view/'.$alert->id;
		}
		return $alerts;
	}
