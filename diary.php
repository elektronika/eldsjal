...
if( isset( $_GET['mode'] ) && $_GET['mode'] != "list" && $_GET['mode'] != "" ) {
	?>
		...
		<h4 class="boxContent">
			<?php require_once( 'addGBEntry.applet.php' );?>
		</h4>
		</div>		
		<?php
}
...
elseif( isset( $_GET['mode'] ) && $_GET['mode'] == "readDiary" ) {
	...
	if( $_SESSION['userid'] == $dbDiary['userid'] ) {
			$sql = "insert into history (action, userid, nick, message, [date], security) values ('diaryread',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." l&auml;ser sin egen tanke \"".$dbDiary['diarytopic']."\" ', getdate(), 0)";
	}
	else {
		$sql = "insert into history (action, userid, nick, message, [date], security) values ('diaryread',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." l&auml;ser ".$dbDiary['username']."s tanke \"".$dbDiary['diarytopic']."\" ', getdate(), 0)";
	}
	...
	print "<tr><td class=diary><b>".rq( $dbDiary['diarytopic'] )."</b><div align=right><a href=diary.php?userid=".$_GET['userid']." class=a2>&laquo; tillbaka</a></div></td></tr>";
...
<?php 
  if( $_SESSION['userid'] == "" || $_SESSION['userid'] == 0 ) {
	...
	if( isset( $_GET['mode'] ) && $_GET['mode'] == "update" ) {
		...
		$sql = "insert into history (action, userid, nick, message, [date], security) values ('diaryupdate',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." skriver om sin tanke \"".$diary['diarytopic']."\" ', getdate(), 0)";
		$result = $conn->execute( $sql );
		$diary = null;
		$result = null;
	}
		...
		//Logger
		$sql = "insert into history (action, userid, nick, message, [date], security) values ('diarynew',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." skriver en ny tanke', getdate(), 0)";
		$result = $conn->execute( $sql );
		$result = null;
	}
...
elseif( isset( $_GET['mode'] ) && $_GET['mode'] == "list" ) {
	...
	//Logger
	$sql = "insert into history (action, userid, nick, message, [date], security) values ('diarylistall',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." listar de senaste 50 tankarna', getdate(), 0)";
	
	$conn->execute( $sql );
	$iCount = 0;
	
	$itemCount = count($dbDiarys);
	makePager("diary.php?mode=list&page=", $itemCount, $page);
	...
	if( !$dbDiarys ) {
		...
		if( $_SESSION['userid'] != "" ) {
			if( $dbdiarys['userid'] = $_SESSION['userid'] ) {
				$sql = "insert into history (action, userid, nick, message, [date], security) values ('diarylistuser',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." listar alla sina tankar', getdate(), 0)";
			}
		}
		else {
			$sql = "insert into history (action, userid, nick, message, [date], security) values ('diarylistuser',".$_SESSION['userid'].", '".$_SESSION['username']."', '".$_SESSION['username']." listar alla tankar skrivna av \"".$dbDiaryss[0]['username']."\" ', getdate(), 0)";
		}
		$result = $conn->execute( $sql );
		$result = null;
	$iCount = 0;
